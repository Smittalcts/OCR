...
class InterviewState(TypedDict):
    ...
    current_question_id: int | None # ADDED: To link Q&A
...
Respond ONLY in the requested JSON format."""),
    ...
    # MODIFIED: Pass the current_question_id to link the answer
    database.add_answer_and_evaluation(
        session_id=state['session_id'],
        question_id=state['current_question_id'], # ADDED
        topic=state['current_topic'],
        user_answer=user_answer,
...
def generate_follow_up(state: InterviewState):
    ...
    # MODIFIED: Pass dynamic question_type and get back the new question_id
    ...
    question_id = database.add_question(
        ...
        question_type=question_type # ADDED
    )
    ...
    return {
        ...
        "follow_up_count": follow_up_num, # MODIFIED
        "api_call_count": 1
    }
...
async def start_practice_interview(request: PracticeRequest, current_user: dict = Depends(auth.get_current_user)):
    ...
    initial_state = InterviewState(
        ...
        current_question_id=None, # ADDED
